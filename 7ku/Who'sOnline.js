// You have a group chat application, but who is online!?
//
// You want to show your users which of their friends are online and available to chat!
//
// Given an input of an array of objects containing usernames, status and time since last activity (in mins),
// create a function to work out who is online, offline and away.
// If someone is online but their lastActivity was more than 10 minutes ago they are to be considered away.
//
// The input data has the following structure:
// [{
//     username: 'David',
//     status: 'online',
//     lastActivity: 10
// }, {
//     username: 'Lucy',
//     status: 'offline',
//     lastActivity: 22
// }, {
//     username: 'Bob',
//     status: 'online',
//     lastActivity: 104
// }]
// The corresponding output should look as follows:
//
// {
//     online: ['David'],
//         offline: ['Lucy'],
//     away: ['Bob']
// }
// If for example, no users are online the output should look as follows:
//
// {
//     offline: ['Lucy'],
//         away: ['Bob']
// }


const users = [
    {
        username: 'David',
        status: 'online',
        lastActivity: 10
    },
    {
        username: 'Lucy',
        status: 'offline',
        lastActivity: 22
    },
    {
        username: 'Bob',
        status: 'online',
        lastActivity: 104
    }
];

const whosOnline = (friends => {
    const userStatus = {
        online: [],
        offline: [],
        away: []
    };
    friends.forEach(itemObject => {
        if (itemObject.lastActivity > 10 && itemObject.status === 'online') {
            userStatus.away.push(itemObject.username);
        } else if (itemObject.status === 'online') {
            userStatus.online.push(itemObject.username);
        } else {
            userStatus.offline.push(itemObject.username);
        }
    });
    Object.keys(userStatus).forEach(key => {
        if (userStatus[key].length === 0) {
            delete userStatus[key];
        }
    });
    return userStatus;
});


whosOnline(users)
console.log(whosOnline(users))
